...
def connect_graph(bot, visit: List[node], start: node, obstacles: List[obstacle]):
    ...
    for goal in visit:
        if goal.grid.x in (1, GRID_X) or goal.grid.y in (
                1, GRID_Y):
            strictness = False
    ...
    for edges in adjM:
        ...
        for edge in range(0, len(visit)):
            ...
            if not nodes[i] == nodes[j]:  # DON'T NEED SOLUTION TO ITSELF
                # mirror case along diagonal
                if adjM[j][i]:
                    adjM[i][j] = adjM[j][i].reverse_path()
                else:
                    adjM[i][j] = astar(bot, nodes[j], obstacles, strictness)
...
def permutate(graph):
    ...
    # new check to see if any goal states cannot be visited by algo (no solution)
    illegal = list()
    ...
    for permutation in permutations:
        ...
        try:
            for goto in permutation:
                ...
                # no solution
                if goto not in illegal and not graph[index][goto]:
                    illegal.append(goto)
                ...
                if index not in illegal and goto not in illegal:
                    ...
                    continue  # do nothing, no path will be added to try and reach this illegal state
...
if __name__ == '__main__':
    n1, n2, n3 = node(pair(2, 3), pair(0, 1)), node(
        pair(8, 8), pair(-1, 0)), node(pair(17, 4), pair(1, 0))
