...
def costing(f):
    ...
    if f.__name__ == 'forward' or f.__name__ == 'back':
        ...
        raise Exception(
            "INVALID MOVEMENT DETECTED FOR COST EVALUATION: %s" % f.__name__)
...
def alt_goal_states(goal: node):
    ...
    if ori in [N, S]:
        # side positions are fine too
        goals = [node(pair(x-1, y), pair(*ori)), goal,
                 node(pair(x+1, y), pair(*ori))]
        for i in range(0, len(goals)):
            ...
            if ori == N:
                ...
                offset = 1
            # 1 tile behind is fine - further from obstacle
            goals.append(node(pair(x, y+offset), pair(*ori)))

    elif ori in [E, W]:
        # side positions are fine too
        goals = [node(pair(x, y-1), pair(*ori)), goal,
                 node(pair(x, y+1), pair(*ori))]
        for i in range(0, len(goals)):
            goal = goals[i]
            x, y = goal.grid.get()
            if ori == E:
                ...
                offset = 1
            # 1 tile behind is fine - further from obstacle
            goals.append(node(pair(x + offset, y), pair(*ori)))

    else:
        raise Exception("Illegal goal state orientation detected: %s" % ori)
...
def perspective(start: node, angle):
    ...
    if start.z == E:
        ...
        return angle  # original was north facing
